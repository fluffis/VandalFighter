...
/*
 * THE CODE BELOW IS VERBATIM FROM browserlauncher.sourceforge.net. IT IS NOT
 * SUBJECT TO THE LICENSING ABOVE -- ITS LICENSE INFO IS DESCRIBED BELOW
 */
/**
 * BbrowserLauncher is a class that provides one static method, openURL, which
		...
		if (osName.startsWith("Mac OS")) {
			...
			try {
				...
				if (version == 2) {
					...
					// Assume that all 2.x versions of MRJ work the same. MRJ
					// 2.1 actually
					...
					// Assume that all 3.1 and later versions of MRJ work the
					// same.
					jvm = MRJ_3_1;
				} else {
	...
	private static boolean loadClasses() {
		...
		case MRJ_3_0:
			try {
				...
				linkage = constructor
						.newInstance(new Object[] { BbrowserLauncher.class });
			} catch (ClassNotFoundException cnfe) {
				errorMessage = cnfe.getMessage();
				return false;
			} catch (NoSuchMethodException nsme) {
			...
			try {
				...
				Object appleEvent = appleEventConstructor
						.newInstance(new Object[] { gurlType, gurlType,
								...
								kAnyTransactionID });
				// Don't set browser = appleEvent because then the next time we
				// call
				// locateBbrowser(), we'll get the same AppleEvent, to which
				// we'll already have
		...
		case MRJ_2_1:
			...
			for (int i = 0; i < systemFolderFiles.length; i++) {
				try {
					...
					// We're looking for a file with a creator code of 'MACS'
					// and
					...
					if (FINDER_TYPE.equals(fileType.toString())) {
						...
						if (FINDER_CREATOR.equals(fileCreator.toString())) {
							browser = file.toString(); // Actually the Finder,
							// but that's OK
							return browser;
						}
					}
